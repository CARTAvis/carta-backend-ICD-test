...
describe(`Spectral profile performance: change thread number per user, ${userNumber} users on 1 backend.`, () => {    
                    ...
                    (threadNumber: number) => {
                        ...
                        async done => {
                            let cartaBackend = await SocketOperation.CartaBackend(
                                baseDirectory, port, userNumber * threadNumber, backendDirectory, readFileTimeout, logMessage);
