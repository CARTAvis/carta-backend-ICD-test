...
describe("PV_GENERATOR_CANCEL: cancel PV request and request it again", () => {
    ...
    describe(`Go to "${assertItem.filelist.directory}" folder`, () => {
        ...
        test(`(Step 5): PV Request, halt PV when progress > 0.5 and check cancel`, async()=>{
            ...
            if (ReceiveProgress < 0.5) {
                ...
                await Connection.send(CARTA.StopPvCalc,assertItem.stopPV);
                let PVResponses = await Connection.streamUntil((type, data) => type == CARTA.PvResponse);
                let PVResponse = PVResponses.PvResponse;
                expect(PVResponse[0].cancel).toEqual(true);
            } 
