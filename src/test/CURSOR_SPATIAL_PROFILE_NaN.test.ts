...
interface AssertItem {
    ...
    assertProfile: {point: {x: number, y: number}, assertPoint: {x: number, y: number}, profileLen: {x: number, y: number}, value: number, oddPoint: {x: {one: {idx: number, value: number}[], others: number|null}, y: {one: {idx: number, value: number}[], others: number|null}}}[];
...
let assertItem: AssertItem = {
    ...
    spatialProfiles: ["x", "y"],
    assertProfile: [
        {point: {x: 314.00, y: 393.00}, assertPoint: {x: 314.00, y: 393.00}, profileLen: {x: 640, y: 800}, value: -0.004026404581964016, oddPoint: {x: {one: [{idx: 0, value: NaN}, {idx: 200, value: -0.0018224817467853427}], others: null}, y: {one: [{idx: 799, value: NaN}, {idx: 400, value: 0.0019619895610958338}], others: null}}},
        {point: {x: 596.00, y: 292.00}, assertPoint: {x: 596.00, y: 292.00}, profileLen: {x: 640, y: 800}, value: NaN, oddPoint: {x: {one: [], others: NaN}, y: {one: [], others: NaN}}},
    ],
    errorPoint: [],
    precisionDigits: 4,
}
    ...
    describe(`read the file "${assertItem.fileName}" on folder "${testSubdirectoryName}"`, () => {
        ...
        assertItem.assertProfile.map( function(item) {
            describe(`set cursor on {${item.point.x}, ${item.point.y}}`, () => {
                    ...
                    test(`Assert value of profile_x : ${item.oddPoint.x.one.map( f => ` #${f.idx} = ${f.value}`)} ${isNaN(item.oddPoint.x.others)?"other values = NaN":isNull(item.oddPoint.x.others)?"":"other values = " + item.oddPoint.x.others}`, () => {
                    ...
                    test(`Assert value of profile_y : ${item.oddPoint.y.one.map( f => ` #${f.idx} = ${f.value}`)} ${isNaN(item.oddPoint.x.others)?"other values = NaN":isNull(item.oddPoint.x.others)?"":"other values = " + item.oddPoint.x.others}`, () => {
